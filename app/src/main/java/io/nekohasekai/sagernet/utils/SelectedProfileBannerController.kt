...
/**
 * Ported from MikuRay's `SelectedProfileBannerController`. Draws a custom
 * (or default) banner image, dimmed for legibility, behind the currently
 * selected profile card in the list — used instead of a plain indicator
 * style when the user enables "Selected banner style".
 */
class SelectedProfileBannerController(private val context: Context) {

    private var changeReceiver: BroadcastReceiver? = null

    fun isEnabled(): Boolean = DataStore.selectedBannerStyleEnabled

    fun hasCustomBanner(): Boolean = DataStore.customSelectedBannerUri.isNotEmpty()

    fun hasBanner(): Boolean = true // always has at least the default banner

    fun applyTo(target: View, cornerRadiusDp: Float = 16f) {
        val uriString = DataStore.customSelectedBannerUri
        if (uriString.isEmpty()) {
            applyDefaultBanner(target, cornerRadiusDp)
            return
        }

        val dimPercent = DataStore.selectedBannerDim.coerceIn(DIM_MIN, DIM_MAX)
    ...
    fun registerChangeListener(onChanged: () -> Unit) {
        ...
        val receiver = object : BroadcastReceiver() {
            override fun onReceive(ctx: Context?, intent: Intent?) {
                if (intent?.action == Action.SELECTED_STYLE_CHANGED) {
                    bitmapCache.clear()
                    onChanged()
                }
            }
        ...
        ContextCompat.registerReceiver(
            context, receiver,
            IntentFilter(Action.SELECTED_STYLE_CHANGED),
    ...
    companion object {
        ...
        private const val DIM_MIN = 0
        private const val DIM_MAX = 90
